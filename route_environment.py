...
state.t_drive = t_drive
----
in update_state_and_trace
    if case(MAKE_A_SPLIT_BREAK):
        t_break = SPLIT_BREAK
...
----
in create_action_set

split_rest_pos = not(state.split_rest) and (SPLIT_REST_TIME < state.t_until_rest)
split_break_pos = (state.t_drive < PARTIAL_DRIVING_MAX) and not(state.break_done)

....

        elif state.event_type == STOP_DRIVING:

            # action_set = {MAKE_A_BREAK}

            action_set = set()
            
            if not state.break_done or (state.t_constant_work >= PARTIAL_DRIVING_MAX and state.t_drive_day < state.t_drive_max):
...
ACTION_NAMES = ['MAKE_A_BREAK', 'FINISH_WORKING_DAY', 'START_REST', 'START_REDUCED_REST',
           ...
           'DRIVE', 'SERVICE', 'MAKE_A_SHORT_BREAK', 'MAKE_A_SPLIT_BREAK', 'DEFAULT']
...
class State:
    ...
    def __str__(self):
        return('event_type: {}, t_simulation: {}, t_current_day: {}, segment: {}, next_node: {}, t_drive_day: {}, t_constant_work: {}, t_until_rest: {}, d_to_next_node: {}, reduced_rest_today: {}, extended_drive_today: {}, t_rest: {}, av_reduced_rest: {}, av_extended_drive: {}'.format(EVENT_NAMES[self.event_type], 
        self.t_simulation, self.t_current_day, self.segment, self.next_node, self.t_drive_day, self.t_constant_work, self.t_until_rest, self.d_to_next_node, self.reduced_rest_today, self.extended_drive_today, self.t_rest, self.av_reduced_rest, self.av_extended_drive))
...
class DeterministicGenerator(BasicGenerator):
    def __init__(self, av_speed = 80, max_speed = 100, min_speed = 60, speed_dev = 5, stime_dev = 0.1, min_serv_time = 0.5):
    ...
    def generate_velocity(self):
        return self.av_speed - self.speed_dev
        
    def generate_service_time(self, av_serv_time):
        if av_serv_time > 0.0:
            serv_time = self.av_serv_time + self.stime_dev * av_serv_time
...
class RouteEnvironment:
    ...
    def reset(self):         
        ...
        # initial trace
        trace = (self.flags+len(self.demands))*[0] # espacio para flags
    ...
    def step(self, state, action, trace):
        while True:
            previous_event = state.event_type
            ...
            if len(action_set) > 1 or state.end == True:
                ...
                print(' WARNING: action_set no definido ')
                print(' >>> Previous event: ' + EVENT_NAMES[previous_event])
                print(' >>> Action : ' + ACTION_NAMES[action])
                print(' >>> Next event: ' + EVENT_NAMES[state.event_type])
    ...
    def execute_action(self, state, action, trace):
        ...
        for case in switch(current_event): # events where actions can be selected
            ...
            if case(SERVICE_AT_NODE):
                ...
                break
            if case():
                print(' ERROR: state.event_type no definido ')
                break
        return state, trace
    ...
    def update_state_and_trace(self, state, action, trace):
        ...
        for case in switch(action):
            ...
            if case(SERVICE):
                # uptate until time window opening 
                path_index = self.path.index(state.next_node)
        ...
        if (state.event_type == ARRIVAL_TO_NODE):
            ...
            if (time_to_open <= 0.0):
                if (state.t_constant_work + service_time < CONTINOUS_WORK_MAX):
                    ...
                    action_set.add(MAKE_A_BREAK)
                    if split_break_pos:
                        action_set.add(MAKE_A_SPLIT_BREAK)
            else:   
                ...
                if (time_to_open > t_break): #  break is possible
                    action_set.add(MAKE_A_BREAK)
                
                if (time_to_open > SPLIT_BREAK) and split_break_pos: #  break is possible
                    action_set.add(MAKE_A_SPLIT_BREAK)
    ...
    def base_policy(self, state):
        ...
        if (state.event_type==STOP_DRIVING):
            ...
            if(state.t_drive_day < state.t_drive_max) and (state.t_until_rest > 0.0): # stop
                decision = MAKE_A_BREAK
            elif((state.t_until_rest + state.t_rest + t_drive_estimated)< time_to_open): # rest at the end of the working day
                decision = FINISH_WORKING_DAY
            elif((state.t_rest + t_drive_estimated) < time_to_open): # rest right now
                decision = START_REST
            elif(((state.t_rest + t_drive_estimated + service_time) > time_to_deadline) and (reduce_rest_pos or extend_drive_pos)): # if driver rests cannot deliver on time
                if(((DAILY_REDUCED_REST + t_drive_estimated) < time_to_deadline) and reduce_rest_pos): # if a shorter rest is enough then do it
                    decision = START_REDUCED_REST
                elif((t_drive_estimated < t_extended_drive) and extend_drive_pos): 
                # if driving one additional hour is enough then do it
                    decision = EXTEND_DRIVE
                elif(extend_drive_pos and reduce_rest_pos):
                # driving one addtional hour implies reducing rest
                    decision = EXTEND_DRIVE_AND_REDUCE_REST
                else:
                    decision = START_REST # no more options
            else: 
                decision = START_REST
            ...
            if((time_to_open > state.t_rest) or (time_to_open + service_time > maximum_t_until_rest)): #  there is time for the daily rest or daily rest must be done
                decision = START_REST       
            elif((time_to_open > SPLIT_REST_TIME) and split_rest_pos): # split rest is convenient
                decision = MAKE_SPLIT_REST
            elif(time_to_open > t_break): #  there is time for a break
                 decision = MAKE_A_BREAK
            elif(time_to_open > SPLIT_BREAK) and split_break_pos: #   there is time for a split break
                 decision = MAKE_A_SPLIT_BREAK
            elif(state.t_constant_work + service_time < CONTINOUS_WORK_MAX):  # serve now
                decision = SERVICE
            else:  # break must be done
                decision = MAKE_A_BREAK
            ...
            if(state.t_until_rest > 0.0) and (state.t_drive_day < state.t_drive_max): # keep working 
                if (state.t_constant_work < CONTINOUS_WORK_MAX): # can drive for a while
                    decision = DRIVE
                else:
                    decision = MAKE_A_BREAK
            else:    # rest
        ...
        if (decision == MAKE_A_BREAK) and split_break_pos:
            decision = MAKE_A_SPLIT_BREAK
    
        return decision

    def simple_base_policy(self, state):
        ...
        if (state.event_type==STOP_DRIVING):
            ...
            if((time_to_open > state.t_rest) or (time_to_open + service_time > maximum_t_until_rest)):
                ...
                decision = MAKE_SPLIT_REST
                 ...
                 decision = MAKE_A_BREAK
            elif(time_to_open > SPLIT_BREAK) and split_break_pos: 
                 decision = MAKE_A_SPLIT_BREAK
    ...
    def dumb_base_policy(self, state):
...
if __name__== "__main__":
    ...
    for step in range(10):
        ...
        print('decisions :' + str(itemgetter(*decisions)(ACTION_NAMES)))
